...
"Estimate the QPS" is three questions in one trench coat: how many requests arrive at the worst moment, how many of those are reads versus writes, and what you're counting as a request in the first place. Answer all three and rung 2 of [the Ladder](../part-1-foundations/04-the-ladder.md) is done. Miss any one and the error compounds through every rung below it — server counts, shard counts, the bill. Traffic numbers are the most load-bearing numbers in the whole estimation, which is why they get a chapter before storage does.
...
> ⚡ **Instinct check** — A government-services portal: 5M DAU, 4 actions each. Average QPS, before reading on. … 5 M × 4 = 20 M ÷ 10^5 = **200 QPS**. More than ten seconds? Redo the [chapter 2](../part-1-foundations/02-the-arithmetic-of-scale.md) drills first.
...
One product-level action explodes at the API tier. Open a feed and the client fires the session refresh, the feed call, per-item hydration, counters, the notification badge, ads, telemetry — a modern page load is ~10–30 API calls. At the other extreme, [typeahead](../part-3-walkthroughs/18-typeahead.md) makes every keystroke a request: typing "biryani near me" is 15 requests before dinner is even chosen. Neither count is wrong; they're counts at different layers, and the gap between them is 10×. The fix costs one spoken sentence — name the unit and the multiplier: "20M DAU × 20 screens a day, and I'll take ~5 API calls per screen — 2B requests/day at the gateway." Interviewers probe exactly this seam.
...
Everything here is [cheat-sheet](../appendices/a-cheat-sheet.md) canon except the API multiplier, which is an assumption you state per system — out loud.
...
Two multipliers to state before anyone relaxes. First, that 30k is the *feed service*; the gateway also carries every sidecar call — at ~10 calls per open, ~100k QPS average and 300k peak — so say which tier you're sizing. Second, 300 posts/s is ingest, not work: each post fans out toward followers' feeds, and the write you accept is not the write you store — the [news-feed walkthrough](../part-3-walkthroughs/12-news-feed.md) does that multiplication. **So the read path is the design problem — 30k QPS of repetitive, cacheable reads wants replicas plus a cache sized to the hot 20% — while the write path is trivial at the front door and explosive behind it, which is why feed design is fan-out design.**
...
Chat's ~1:1 ratio triples after fan-out — and the edge isn't sized by any per-second rate at all but by 5M held-open sockets, ~50 boxes at 100k connections each ([chat walkthrough](../part-3-walkthroughs/13-chat.md)).
</details>
...
---
[← Previous: The Ladder](../part-1-foundations/04-the-ladder.md) · [Table of contents](../../README.md) · [Next: Storage →](06-storage.md)
