...
Every value below is quoted from [Appendix A](a-cheat-sheet.md) — the one page this book asks you to own.
...
One reread of the table pays immediately: the Redis row equals the network row. A cache hit is a *network operation* — the microseconds of hashing vanish inside the 0.5 ms round trip. [Memory & cache](08-memory-and-cache.md) builds on exactly this.
...
The only family whose whys are about humans rather than silicon — so each rationale fits in a breath. ([Traffic](05-traffic.md) derives the peak factors properly.)
...
**Spaced repetition for the values.** The [Anki deck](https://github.com/pgcurious/estimation-instinct/blob/main/drills/anki-anchor-numbers.csv) carries the anchors as flashcards — ten minutes a day while the kettle boils. Necessary, not sufficient: cards build recognition, and the interview tests recall, out loud.

**Blank-page reproduction for the structure.** Once a week, write the ladder and the ceilings from memory, then diff against [the cheat sheet](a-cheat-sheet.md). [The 30-day program](24-the-30-day-program.md) puts a blank page in front of you on day 7 and assumes the full sheet from memory by day 10. The diff is the lesson: the rung you misplace is the rung you never attached a why to.
...
**Drill 3.1** — Blank page, five minutes: reproduce the latency ladder from the desk to another continent — every rung you can, with values. Then diff against [Appendix A](a-cheat-sheet.md).
